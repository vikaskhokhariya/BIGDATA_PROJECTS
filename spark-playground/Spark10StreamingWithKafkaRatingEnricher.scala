...
object Spark10StreamingWithKafkaRatingEnricher extends App with Base {

  val spark = SparkSession.builder().appName("Spark Streaming with Kafka").master("local[*]").getOrCreate()
  ...
  def processAndEnrich(ratingRdd: RDD[String]): Unit = {
    ...
    ratingDF.join(movieDF, "mID").write.mode(SaveMode.Append).json("")
  }
